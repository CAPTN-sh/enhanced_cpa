...
from darts.models import KalmanForecaster, AutoARIMA, ExponentialSmoothing, StatsForecastAutoCES, StatsForecastAutoTheta, FourTheta

# Make a prediction by the Triple Exponential Smoothing method like described by Sang et.al.
...
def dynamic_prediction(ship, past_timesteps, forecast_steps, prediction_method):
    ...
    if prediction_method == 'double_exponential':
        for i in range(len(data)):  # Loops two times, one for speed and one for course
        ...
        predict_speed = forecast_result[0]  # All the predicted speeds
        predict_course = forecast_result[1]  # All the predicted course
    ...
    # Use Kalman Filter
    elif prediction_method == 'kalman_filter':
        for i in range(len(data)):
            series = TimeSeries.from_values(np.array(data[i]))
            model = KalmanForecaster(dim_x=10)
            mean_value = np.mean(series.values())
            if np.isclose(mean_value, 0):
                # Add a small offset to avoid zero mean
                series += 1e-5
            model.fit(series)
            forecast = model.predict(n=forecast_steps)
            forecast_result.append(forecast.values().flatten())
        # Save resulting predictions
        predict_speed = forecast_result[0]
        predict_course = forecast_result[1]
    ...
    # Use Stats Forecast AutoCES
    elif prediction_method == 'stats_forecast_auto_ces':
        for i in range(len(data)):
            series = TimeSeries.from_values(np.array(data[i]))
    ...
    # Use Stats Forecast AutoTheta
    elif prediction_method == 'stats_forecast_auto_theta':
        for i in range(len(data)):
            series = TimeSeries.from_values(np.array(data[i]))
    ...
    # Use Theta Method
    elif prediction_method == 'four_theta':
        for i in range(len(data)):
            series = TimeSeries.from_values(np.array(data[i]))
            model = FourTheta()
            mean_value = np.mean(series.values())
